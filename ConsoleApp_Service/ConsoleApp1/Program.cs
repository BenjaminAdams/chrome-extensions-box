...
using ConsoleApp1.ConfigService;
using Dell.Cmp.DataContracts.Payloads;
            ...
            foreach (var data in configData)
            {
                if (data.AlertOnChange)
                {
                    ...
                    if (data.SetPrice <= configPrice && !(configPrice <=0))
                    {
                        ...
                        var postResult  =  program.CreateOrder(jarrayObject);
                        Email();


                        Console.WriteLine(data.ConfigItem, data.SetPrice, configPrice);
                    }
                }
            }
            Console.ReadLine();
